...
def DetectQR(img, image_y, image_x):
    ...
    if len(lines) == 4:
        ...
        return [CountIntersection(lines[0], lines[1]),
                  CountIntersection(lines[1], lines[2]),
    ...
    #     if line[0] == math.inf:
    #         continue
    #     cv2.line(line_image, (line[0], line[1]), (line[0], line[1]), (255, 0, 0), 3)
    # cv2.imshow('sd', line_image), cv2.waitKey(0)
    return answer
...
def DetectDM(img, gray, image_y, image_x):
    ...
    for line in lines:
        for x1, x2, y1, y2 in line:
            cv2.line(line_image, (x1, x2), (y1, y2), (255, 0, 0), 3)
    #cv2.imshow('sd', line_image), cv2.waitKey(0)
    ...
    #     if line[0]==math.inf:
    #         continue
    #     cv2.line(line_image, (int(line[0]), int(line[1])), (int(line[0]), int(line[1])), (255, 0, 0), 3)
    # cv2.imshow('sd', line_image), cv2.waitKey(0)
    # scr = np.array([[0, 0], [0, image_y], [image_x, image_y]]).astype(np.float32)
    # dst = np.array(answer[:3]).astype(np.float32)
    # mat = cv2.getAffineTransform(scr, dst)
...
for object in data['objects']:
    ...
    img = img[bboxes[1]:bboxes[1]+bboxes[3],bboxes[0]:bboxes[0]+bboxes[2]]
    points = Normalize(img, type)
    if points == 0:
        print('Failed')
        continue
    flag = False
    ...
    if flag:
        continue
    diff = [CountDistance(points[i][0], points[i][1], ploc[2*i], ploc[2*i+1]) for i in range(4)]
    summ += sum(diff)
    count += 4
    if all(diff) < 10:
        ok+=1
